...
def filter_invalid(bbox, label=None, score=None, mask=None, thr=0.0, min_size=0, reverse=False, topk=-1):
    if score is not None:
        if reverse:
            valid = score <= thr
        else:
            valid = score > thr
        if topk > 0 and torch.nonzero(valid).shape[0] > topk:
            topk_scores, _ = torch.topk(score[valid], topk, largest=True, sorted=False)
            valid = score >= topk_scores.min()
        bbox = bbox[valid]
    ...
    if min_size is not None:
        ...
        if mask is not None:
            mask = BitmapMasks(mask.masks[valid.cpu().numpy()], mask.height, mask.width)
    return bbox, label, mask, valid.nonzero().squeeze(1)
