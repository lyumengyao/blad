"""
Modified from https://github.com/microsoft/SoftTeacher/blob/main/ssod/datasets/pipelines/rand_aug.py.
"""
import copy
...
@PIPELINES.register_module()
class MultiBranchTest(object):
    def __init__(self, aug_times=1, **transform_group):
        self.transform_group = [{k: BaseCompose(v) for k, v in transform_group.items()} for _ in range(aug_times)]

    def __call__(self, results):
        multi_results = []
        for d in self.transform_group:
            for k, v in d.items():
                res = v(copy.deepcopy(results))
                if res is None:
                    return None
