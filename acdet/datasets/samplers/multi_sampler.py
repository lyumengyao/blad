...
class DistributedGroupMultiBalanceSampler(Sampler):
    def __init__(
        self,
        dataset,
        by_prob=False,
        ...
        **kwargs
        ...
        for i, _ in enumerate(self.group_sizes):
            ...
            for j in range(len(self.cumulative_sizes)):
                ...
                # use the largest size among different data types 
                ...
                while batch_idx < self.epoch_length[i] * self.num_replicas:
                    ...
                    if self.by_prob:
                        ...
                        for u, c in zip(unique, counts):
                            ratio[u] = c
                        for idx in np.where(np.array(ratio) == 0)[0]:
                            max_counts = max(ratio)
                            if max_counts <= 1:
                                raise NotImplementedError
                            max_ind = ratio.index(max_counts)
                            ratio[max_ind] -= 1
                            ratio[idx] += 1
